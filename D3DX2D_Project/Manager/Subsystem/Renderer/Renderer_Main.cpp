...
void Renderer::RenderMain(class Actor* const actor, class IRendererComponent* component, const RenderableType& type)
{
	...
	if (pipeline->Begin(pipeline_state))
	{
		...
		if (animator && type == RenderableType::Opaque)
		{
			...
			cpu_texture_buffer.sprite_offset = current_keyframe->offset;
			cpu_texture_buffer.sprite_size = current_keyframe->size;
			cpu_texture_buffer.color_key = current_animation->GetColorKey();
			cpu_texture_buffer.texture_size = animator->GetCurrentAnimation()->GetSpriteTextureSize();
			cpu_texture_buffer.is_texture = 1.0f;
			UpdateTextureBuffer();

			pipeline->SetConstantBuffer(2, vertex_shader_scope | pixel_shader_scope, gpu_texture_buffer.get());
		...
		else if (texture && type == RenderableType::Opaque)
		{
			...
			cpu_texture_buffer.sprite_size = current_texture->GetSize();
			cpu_texture_buffer.color_key = current_texture->GetColorKey();
			cpu_texture_buffer.texture_size = current_texture->GetSpriteTextureSize();
			cpu_texture_buffer.is_texture = 1.0f;
			UpdateTextureBuffer();

			pipeline->SetConstantBuffer(2, vertex_shader_scope | pixel_shader_scope, gpu_texture_buffer.get());
			pipeline->SetShaderResource(0, pixel_shader_scope,
				current_texture->GetSpriteTexture().get());
		}

		else if (collider)
		{
			cpu_texture_buffer.sprite_offset = D3DXVECTOR2(0, 0);
			cpu_texture_buffer.sprite_size = D3DXVECTOR2(1, 1);
			cpu_texture_buffer.texture_size = D3DXVECTOR2(1, 1);
			cpu_texture_buffer.is_texture = 0.0f;
			UpdateTextureBuffer();
